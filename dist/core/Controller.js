'use strict';Object.defineProperty(exports,'__esModule',{value:true});exports.default=undefined;var _View=require('./View');var _View2=_interopRequireDefault(_View);var _Observer=require('../patterns/observer/Observer');var _Observer2=_interopRequireDefault(_Observer);function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj}}function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError('Cannot call a class as a function')}}/**
 ...
 */var Controller=function(){/**
   ...
   * @protected
   * @type {Object}
   */function Controller(key){_classCallCheck(this,Controller);if(Controller.instanceMap[key]){throw new Error(Controller.MULTITON_MSG)}/**
     ...
     * @protected
     * @type {string}
     */this.multitonKey=key;Controller.instanceMap[this.multitonKey]=this;/**
   ...
   * @protected
   * @type {string}
   */Controller.prototype.initializeController=function initializeController(){this.view=_View2.default.getInstance(this.multitonKey)};/**
   ...
   */Controller.getInstance=function getInstance(key){if(!key){return null}if(!Controller.instanceMap[key]){return new Controller(key)}return Controller.instanceMap[key]};/**
   ...
   * @return {void}
   */Controller.prototype.executeCommand=function executeCommand(note){if(!note){return}var commandClassRef=this.commandMap[note.getName()];if(!commandClassRef){return}var commandInstance=new commandClassRef;commandInstance.initializeNotifier(this.multitonKey);commandInstance.execute(note)};/**
   ...
   * @return {void}
   */Controller.prototype.registerCommand=function registerCommand(notificationName,commandClassRef){if(!this.commandMap[notificationName]){this.view.registerObserver(notificationName,new _Observer2.default(this.executeCommand,this))}this.commandMap[notificationName]=commandClassRef};/**
   * Check if a command is registered for a given Notification
   *
   * @param {string} notificationName
   * @return {boolean}
   *  whether a Command is currently registered for the given notificationName.
   */Controller.prototype.hasCommand=function hasCommand(notificationName){return!!this.commandMap[notificationName]};/**
   ...
   * @return {void}
   */Controller.prototype.removeCommand=function removeCommand(notificationName){if(this.hasCommand(notificationName)){this.view.removeObserver(notificationName,this);delete this.commandMap[notificationName]}};/**
   * @static
   ...
   * @return {void}
   */Controller.removeController=function removeController(key){delete this.instanceMap[key]};return Controller}();Controller.instanceMap={};Controller.MULTITON_MSG='controller key for this Multiton key already constructed';exports.default=Controller;module.exports=exports['default'];